...
if (cmd === "song" || cmd === "yta") { if (!text) { return sock.sendMessage(m.from, { text: "🔎 Please provide a song name or YouTube link!" }, { quoted: m }); }
...
try {
  ...
  if (!result.success || !result.result || !result.result.downloadUrl) {
    return sock.sendMessage(m.from, { text: "❌ Failed to fetch download link!" }, { quoted: m });
  }

  const { title, downloadUrl, image } = result.result;
