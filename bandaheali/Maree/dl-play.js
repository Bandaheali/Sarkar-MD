...
// Optimized API Configuration with 4 high-quality APIs
const APIS = [
  {
    name: "AgatzAPI",
    url: (url) => `https://api.agatz.xyz/api/ytmp3?url=${encodeURIComponent(url)}`,
    getUrl: (data) => {
      // Select highest quality (192kbps if available)
      const bestQuality = data?.data?.reduce((prev, current) => 
        (parseInt(current.quality) > parseInt(prev.quality)) ? current : prev
      );
      return bestQuality?.downloadUrl;
    },
    timeout: 4000
  },
  {
  ...
  {
    name: "FgsiRestAPI",
    url: (url) => `https://fgsi1-restapi.hf.space/api/downloader/youtube/v1?url=${encodeURIComponent(url)}&format=mp3`,
...
const dlplay = async (m, sock) => {
  ...
  try {
    ...
    if (!audioUrl) throw new Error("All APIs failed to respond in time");
